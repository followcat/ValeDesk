...
export function TaskDialog({
  ...
  const generateTaskTitle = (
    targetMode: TaskMode,
    targetTasks: ThreadTask[],
    targetConsensusPrompt: string,
    targetConsensusQuantity: number
  ): string => {
    if (targetMode === "consensus") {
      if (targetConsensusPrompt.trim()) {
        const words = targetConsensusPrompt.trim().split(/\s+/).filter(word => word.length > 0);
        if (words.length > 0) {
          const firstWord = words[0].replace(/[^a-zA-Zа-яА-Я0-9]/g, '');
          return t("taskDialog.consensusPromptTitle", { word: firstWord, count: targetConsensusQuantity });
        }
      }
      return t("taskDialog.consensusTitle", { count: targetConsensusQuantity });
    }

    const uniqueModels = [...new Set(targetTasks.filter(t => t.model).map(t => t.model))];
    if (uniqueModels.length > 0) {
      if (uniqueModels.length === 1) {
        return t("taskDialog.singleModelTitle", { model: uniqueModels[0], count: targetTasks.length });
      }
      return t("taskDialog.multiModelTitle", { count: targetTasks.length });
    }
    return t("taskDialog.multiTaskTitle");
  };
  ...
  const handleCreateTask = () => {
    const title = generateTaskTitle(mode, tasks, consensusPrompt, consensusQuantity);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/20 px-4 py-8 backdrop-blur-sm">
      <div className="w-full max-w-2xl rounded-2xl border border-ink-900/5 bg-surface p-6 shadow-elevated max-h-[90vh] overflow-y-auto">
        ...
        <p className="mt-2 text-sm text-muted">{t("taskDialog.subtitle")}</p>

        <div className="mt-5 grid gap-4">
          {/* Auto-generated Title */}
          <div className="grid gap-1.5">
            <span className="text-xs font-medium text-muted">{t("taskDialog.taskTitleLabel")}</span>
            <div className="rounded-xl border border-ink-900/10 bg-surface-secondary px-4 py-2.5 text-sm text-ink-800">
              {generateTaskTitle(mode, tasks, consensusPrompt, consensusQuantity)}
            </div>
          </div>

          {/* Mode Selection */}
          <label className="grid gap-1.5">
            <span className="text-xs font-medium text-muted">{t("taskDialog.executionModeLabel")}</span>
            <div className="grid grid-cols-2 gap-2">
              ...
              >
                ...
                <div className="text-xs mt-1">{t("taskDialog.consensusModeHint")}</div>
              </button>
              <button
                type="button"
              ...
              >
                <div className="font-medium">{t("taskDialog.differentTasksMode")}</div>
                <div className="text-xs mt-1">{t("taskDialog.differentTasksModeHint")}</div>
              </button>
            </div>
          </label>
          ...
          <label className="grid gap-1.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted">{t("taskDialog.workspaceFolderLabel")}</span>
            ...
            <div className="flex gap-2">
              <input
                ...
                placeholder={t("taskDialog.workspaceFolderPlaceholder")}
                value={localCwd}
            ...
            <p className="text-[11px] text-muted-light">
              {t("taskDialog.workspaceFolderHint")}
            </p>
            {recentCwds.length > 0 && (
              <div className="mt-2 grid gap-2 w-full">
                <div className="text-[11px] font-medium uppercase tracking-wide text-muted-light">{t("taskDialog.recentLabel")}</div>
          ...
          {mode === "consensus" && (
            <>
              {/* Model */}
              <label className="grid gap-1.5">
                <span className="text-xs font-medium text-muted">{t("taskDialog.modelForAllThreadsLabel")}</span>
                <DropdownMenu.Root>
              ...
              {/* Quantity */}
              <label className="grid gap-1.5">
                <span className="text-xs font-medium text-muted">{t("taskDialog.threadCountLabel")}</span>
                <input
                  type="number"
                  min="2"
                  max="10"
                ...
                />
                <p className="text-[11px] text-muted-light">{t("taskDialog.threadCountHint")}</p>
              </label>

              {/* Prompt */}
              <label className="grid gap-1.5">
                <span className="text-xs font-medium text-muted">{t("taskDialog.taskDescriptionLabel")}</span>
                <textarea
                  rows={4}
            ...
            <div className="grid gap-4">
              {tasks.map((task, index) => (
                <div key={task.id} className="rounded-xl border border-ink-900/10 bg-surface-secondary p-4">
                  ...
                  {/* Model */}
                  <label className="grid gap-1.5">
                    <span className="text-xs font-medium text-muted">{t("taskDialog.modelLabel")}</span>
                    <DropdownMenu.Root>
                  ...
                  <label className="grid gap-1.5 mt-3">
                    <span className="text-xs font-medium text-muted">{t("taskDialog.taskLabel")}</span>
                    <textarea
                      rows={3}
