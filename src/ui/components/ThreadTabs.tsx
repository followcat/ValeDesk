...
export function ThreadTabs({ threads, activeThreadId, onThreadSelect, layout = 'tabs' }: ThreadTabsProps) {
  ...
  const getStatusLabel = (status: ThreadInfo['status']) => {
    switch (status) {
      case 'running': return t("threadTabs.status.running");
      case 'completed': return t("threadTabs.status.completed");
      case 'error': return t("threadTabs.status.error");
      default: return t("threadTabs.status.idle");
    }
  };
      ...
      <div className="flex gap-4 overflow-x-auto pb-2">
        {sortedThreads.map((thread) => (
          ...
          >
            ...
            <div className="text-xs text-muted mt-1">{t("threadTabs.messageCount", { count: thread.messageCount })}</div>
            {thread.hasChanges && (
              <div className="text-xs text-accent mt-1 flex items-center gap-1">
                <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
                {t("threadTabs.hasChanges")}
              </div>
            )}
          </div>
        ))}
